...
class UpdateChecker:
    ...
    def print_summary(self, stats_list: List[Dict]):
        ...
        if self.new_measures:
            print("\n🆕 Sample of New Measures (first 5):")
            for item in self.new_measures[:5]:
                measure = item['measure']
                print(f"  • [{item['source']}] {measure.get('year', 'N/A')} - {measure.get('title', 'Unknown')[:60]}")
        ...
        if self.updated_measures:
            print("\n🔄 Sample of Potential Updates (first 5):")
            for item in self.updated_measures[:5]:
                measure = item['measure']
                existing = item['existing']
                print(f"  • [{item['source']}] {measure.get('title', 'Unknown')[:60]}")
