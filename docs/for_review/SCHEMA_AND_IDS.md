...
>
> **Most changed here:** the finance schema was rebuilt (year-scoped
> `finance_campaign_id`), and the registrar loader adds three tables —
> `registrar_load_scopes`, `registrar_identities`, and
> `registrar_identity_aliases`. Registrar measures use explicit
> `REG_<COUNTY>_<YYYYMMDD>_<sha256>` identifiers rather than derived ones.
