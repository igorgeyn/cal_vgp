...
>
> **Most changed here:** county registrars are now a data source, supplying
> official primary documents (notices, resolutions, full texts, impartial
> analyses, tax rate statements, arguments) for local measures.
...
**Coverage:** Statewide propositions + all 58 counties (2006-2025 in active DB)
...
### 6. UC Law SF (Web Scraping)

**Coverage:** Statewide propositions (2010-2018 in active DB)

**Scraper:** Configured in `src/config.py:49-54`

**Website:** repository.uclawsf.edu

**Data Collected:**
- Proposition text and metadata from Hastings scholarship archive

**Output:** `data/raw/UC_Law_SF_*.json` (6 files)

**Active DB Records:** 2

> _Evidence: `SELECT COUNT(*) FROM active_measures WHERE data_source='UC_Law_SF';` → 2._

---

### 7. County-Specific Scrapers
...
- **Size:** ~6.1 MB
- **Format:** Array of measure objects (21 fields)

**Summary Quality Export:**
- **File:** `scraper/data/exports/summary_quality_YYYYMMDD.csv`
- **Script:** `scripts/export_data.py --format summary-quality`
- **Purpose:** Quality analysis of AI-generated summaries (length, refusal detection)
- **Scope:** Measures from 2020+ with summaries

### Processed Data
- **`scraper/data/processed/ceda_parsed.csv`** — Parsed CEDA archive (11,050 rows including header)
- **`scraper/data/processed/ceda_parsed.json`** — Same data in JSON format
- **Pipeline:** Generated by `src/parsers/ceda.py` from raw CEDA files
